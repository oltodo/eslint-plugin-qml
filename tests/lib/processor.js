...
describe("processor", function() {
  describe("preprocess", function() {
    ...
    it("should return an array", function() {
      assert.isArray(processor.preprocess(getCode()));
    });

    it("should ignore normal text", function() {
      var blocks = processor.preprocess(getCode());

      assert.equal(blocks.length, 0);
    });

    it("should return code into property", function() {
      var blocks = processor.preprocess(
        getCode("property var hello: 'Hello World!'\n")
      );

      assert.equal(blocks.length, 1);
      assert.equal(blocks[0], "'Hello World!'\n");
    });

    it("should return function code", function() {
      var blocks = processor.preprocess(
        getCode("function hello () {", "console.log('Hello World!')", "}\n")
      );
